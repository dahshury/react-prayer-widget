...
import { getAzanSource, type PrayerName } from "@/lib/azan";
...
export function useAzan({ prayerTimes, settings }: UseAzanArgs) {
	...
	useEffect(() => {
		if (!settings.azanEnabled || schedule.length === 0) return;
		...
		const playFor = (prayer: PrayerName) => {
			...
			if (lastPlayedRef.current === marker) return;
		...
		const scheduleNext = () => {
			...
			if (!next) next = schedule[0]; // next day fajr
			const target = new Date();
			const [h, m] = next.hhmm.split(":").map(Number);
			target.setHours(h, m, 0, 0);
			if (
				next === schedule[0] &&
				schedule[schedule.length - 1].minutes < nowM
			) {
